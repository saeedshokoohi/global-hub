...
import com.eyeson.global_hub.config.KafkaProperties;
...
class GlobalHubKafkaResourceIT {
    ...
    void setup() {
        ...
        GlobalHubKafkaResource kafkaResource = new GlobalHubKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/global-hub-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/global-hub-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
